...
static void* RecordThread(void* param)
{
	int ret = 0;
	char *buffer = NULL;
	char *resample_inbuff=NULL;
	char *resample_outbuff=NULL;
	int i =0;
	ALOGE("-lfy-RecordThread start 1 \n");
	if(g_record_event != NULL){
		native_event_wait(g_record_event, 0x7fffffff); 
	}
	
	//speex_resampler =NULL;
	//speex_resampler = speex_resampler_init(2, 48000, 16000, RESAMPLER_QUALITY_DEFAULT, NULL);//初始化 
	ALOGE("-lfy-RecordThread start 2 \n");
	...
	while(1){
		if(in_pcm_handle != NULL)
		{
			...
			if(audio_record_status == 1)
			{

	            //ALOGE("channel_mask = AUDIO_CHANNEL_IN_MONO");
	           // for (i = 0; i < in_buff_size/2; i++)
	            //    buffer[i] = buffer[i * 2];
				
				share_write_data(buffer, in_buff_size);
			//	speex_resampler_process_interleaved_int(speex_resampler,buffer,&in_len,resample_outbuff,&out_len);
			}
#if SK04_WAKE_UP_DEBUG		
...
static void start_record()
{
	...
	if( (in_pcm_count==1) && (in_pcm_handle==NULL) ){
		ALOGD("-lfy-start_wake_up record ");
		in_pcm_handle =  pcm_open(USB_AUDIO_CARD, PCM_DEVICE, PCM_IN, &config);
		if (!pcm_is_ready(in_pcm_handle)) {
			ALOGE("-lfy-cannot open pcm_in driver: %s", pcm_get_error(in_pcm_handle));
			pcm_close(in_pcm_handle);
			in_pcm_handle = NULL;
			return;
		}
		...
		in_buff_size =1024;//pcm_frames_to_bytes(in_pcm_handle, config.period_size);	
		ALOGD("-lfy-pcm_frames_to_bytes =%d ",pcm_frames_to_bytes(in_pcm_handle, pcm_get_buffer_size(in_pcm_handle)));
...
*/
	...
	share_write_init();
	native_event_set(g_record_event);
}

static void stop_record()
{
	ALOGD("-lfy-stop_record last record ");		
#if	SK04_WAKE_UP_DEBUG
...
static int start_input_stream(struct stream_in *in)
{
	...
	ALOGE("pcm_open() USB_AUDIO_CARD: %d", USB_AUDIO_CARD);
    in->pcm = pcm_open(USB_AUDIO_CARD, PCM_DEVICE, PCM_IN, in->config);
                       ...
                       size_t bytes)
{
	...
	//ALOGE("--lfy-%s:bytes:%d",__FUNCTION__,bytes);
                                   ...
                                   const char *address __unused)
{
    ...
    ALOGE("--lfy--audio hal adev_open_output_stream devices = 0x%x, flags = %d, samplerate = %d",
...
//	ALOGE("--lfy-adev_set_parameters, %s", kvpairs);
	parms 	= str_parms_create_str(kvpairs);
    ...
    if (ret >= 0) {
		...
		if( (temp_val == 1) && (audio_record_status == 0) ) {
				in_pcm_count =1;
			ALOGE("--lfy-adev_set_parameters in_pcm_count =%d", in_pcm_count);
			share_write_clear();
			start_record();
		...
		ALOGE("adev_set_parameters  audio_record_status  =%d,in_pcm_count = %d ", audio_record_status,in_pcm_count);
    }
                                  ...
                                  const char *keys)
{
	...
	if(!strcmp(keys, "audio_record_status")){
		char str[50];
		ALOGE("--lfy-adev_Get_parameters audio_record_status = %d", audio_record_status);
...
static int adev_close(hw_device_t *device)
{
	...
	in_pcm_count = 0;
    pthread_join(tid_pcm_read, NULL);;
	usleep(10*1000);

	ALOGE("---lfy-adev_close  \n");
#endif	
    return 0;
}
...
{
	ALOGD("---lfy--share_buffer_thread start \n");
	share_buffer_init();
	ALOGD("--lfy--share_buffer_thread end \n");
	return NULL;
} 
                     ...
                     hw_device_t** device)
{
    ...
    *device = &adev->hw_device.common;
	...
	pthread_create(&tid_pcm_read, &thread_attr, RecordThread, (void*)in_pcm_handle);
