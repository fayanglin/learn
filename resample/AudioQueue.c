...
void queue_clear(audio_queue_t* queue)
{
	pthread_mutex_lock(&(queue->mutex));
	queue->front = 0;
	queue->rear = 0;
...
int queue_write(audio_queue_t* queue, char data[], int dataLen)
{
	...
	if (queue_left_asyn(queue) < dataLen) {
		pthread_mutex_unlock(&(queue->mutex));
		ALOGV("ljg_queue_left_asyn queue < dataLen ,data too much,space not enough\n");
		return false;
	}
	// 计算数据区起始地址
...
int queue_read(audio_queue_t* queue, char **data,int dataLen)
{
	// 计算数据区起始地址
	char* queueBase = NULL;
	char* begin = NULL;
	...
	native_event_wait(queue->sync_event, 30); 
	pthread_mutex_lock(&(queue->mutex));
	queueBase = (char*)(queue + 1);
	begin = &((queueBase)[queue->front]);
	queueLen = queue_len_asyn(queue);
	//ALOGD("queue_read queueLen = %d\n", queueLen);
	if(queueLen < dataLen){
		//ALOGW("queue_read fail, queueLen < dataLen \n");
		pthread_mutex_unlock(&(queue->mutex));
		return -1;
	}
	queueLen = dataLen;
	...
	if (NULL == data){
		ALOGV("ljg_queue_read malloc error queueLen%d\n", queueLen);
		return 0;
	}
